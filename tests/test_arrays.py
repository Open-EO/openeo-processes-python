...
def test_array_element(xr_array, xr_array_B08):
    ...
    xr.testing.assert_equal(
        oeop.array_element(xr_array, dimension='s', label="B08"),
        xr_array_B08)
    xr.testing.assert_equal(
        oeop.array_element(xr_array, dimension='s', index=0),
        xr_array_B08)
    # Assert raised errors?
    # ArrayElementNotAvailable
    # oeop.array_element(xr_array, dimension='s', label="B09")
    # oeop.array_element(xr_array, dimension='s', index=4)
...
#     .assertListEqual(oeop.order([6, -1, 2, np.nan, 7, 4, np.nan, 8, 3, 9, 9]).tolist(),
#                          [1, 2, 8, 5, 0, 4, 7, 9, 10])
#     .assertListEqual(oeop.order([6, -1, 2, np.nan, 7, 4, np.nan, 8, 3, 9, 9], nodata=True).tolist(),
#                          [1, 2, 8, 5, 0, 4, 7, 9, 10, 3, 6])
#     .assertListEqual(oeop.order([6, -1, 2, np.nan, 7, 4, np.nan, 8, 3, 9, 9], asc=False, nodata=True).tolist(),
#                          [9, 10, 7, 4, 0, 5, 8, 2, 1, 3, 6])
#     .assertListEqual(oeop.order([6, -1, 2, np.nan, 7, 4, np.nan, 8, 3, 9, 9], asc=False, nodata=False).tolist(),
#                          [6, 3, 9, 10, 7, 4, 0, 5, 8, 2, 1])
# 
# def test_rearrange():
#     """ Tests `rearrange` function. """
#     .assertListEqual(oeop.rearrange([5, 4, 3], [2, 1, 0]).tolist(), [3, 4, 5])
#     .assertListEqual(oeop.rearrange([5, 4, 3, 2], [1, 3]).tolist(), [4, 2])
#     .assertListEqual(oeop.rearrange([5, 4, 3, 2], [0, 2, 1, 3]).tolist(), [5, 3, 4, 2])
# 
# def test_sort():
#     """ Tests `sort` function. """
#     .assertListEqual(oeop.sort([6, -1, 2, np.nan, 7, 4, np.nan, 8, 3, 9, 9]).tolist(),
#                          [-1, 2, 3, 4, 6, 7, 8, 9, 9])
